...
_PROMPT_TEMPLATE = """\
これは勤務シフト表のPDFファイルです。この中から、名前が「{name}」の人が出勤する日をすべて抽出してください。

ルール:
- 表の形式は月間カレンダー形式、名前×日付のマトリクス形式など様々な可能性があります。PDF内の表の構造をよく観察してから判断してください。
- PDFが複数ページ、または複数の表を含む場合は、すべてのページを確認してください。
...
def extract_shift_dates_from_pdf(pdf_bytes: bytes, user_name: str) -> dict:
